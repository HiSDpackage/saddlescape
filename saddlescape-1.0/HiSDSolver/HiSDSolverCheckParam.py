...
def HiSDCheckParam(instance, **kwargs):
	...
	if "EnergyFunction" not in kwargs:
		...
		if not isinstance(instance.EnergyFunction, str):
			if callable(instance.EnergyFunction):
...
def auto_checking_parameter(instance, param_name, kwargs, param_value):
	...
	if param_name not in kwargs:
		...
		if param_name == "CustomTD":
			if kwargs["CustomTD"] is None or callable(kwargs["CustomTD"]):
				return kwargs["CustomTD"]
			else:
				raise ValueError(
					f"Invalid `CustomTD` value: expected callable, got {kwargs['CustomTD']} (type={type(kwargs['CustomTD']).__name__})."
				)

		if param_name == "CustomTD_Step":
			if kwargs["CustomTD_Step"] is None:
				return None
			elif (
				isinstance(kwargs["CustomTD_Step"], int)
				and kwargs["CustomTD_Step"] > 0
			):
				return kwargs["CustomTD_Step"]
			else:
				raise ValueError(
					f"Invalid `CustomTD_Step` value: expected positive integer, got {kwargs['CustomTD_Step']} (type={type(kwargs['CustomTD_Step']).__name__})."
				)
