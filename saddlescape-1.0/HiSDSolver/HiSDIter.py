...
def HiSDIteration(instance):
	...
	# Use a user-defined temporal discretization if provided.
	if instance.CustomTD is not None:
		if instance.CustomTD_Step is not None:
			CustomTD_Step = instance.CustomTD_Step
		else:
			CustomTD_Step = None
		glist = [copy.deepcopy(g)]
		xlist = [copy.deepcopy(x)]
		vlist = [copy.deepcopy(v)]
		for j in range(1, instance.MaxIter + 1):
			if WhetherBB and j > 1:
				dt = BBStepSize(x, x_pre, g, g_pre, tau)
			timestep_record.append(dt)

			# Apply the user-defined temporal discretization.
			x_temp, v, whetherkindex = instance.CustomTD(instance, xlist, vlist, glist, dt, j)
...
def SDIteration(instance):
	...
	if instance.CustomTD is not None:
		...
		for j in range(1, instance.MaxIter + 1):
			...
			# Apply the user-defined temporal discretization.
			x_temp = instance.CustomTD(instance, xlist, None, glist, dt, j)
